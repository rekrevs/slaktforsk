# T-NNNN: Avgränsat kunskapsresultat

**Status**: READY | **Size**: M
**Phase**: -

## Context och mandat

Ägar-/Project Control-beslut, varför uppgiften behövs och aktuellt kontrakt.
Läs wotan/README.md och genealogy2/docs/working.md samt normerna
i docs/research/person-contract.md, research-program.md och source-strategy.md. Läs och skriv aktuell
kunskap i genealogy2; gamla profil-/aktfiler är arkivunderlag.

## Scope och befintligt underlag

- Personer/hushåll: fast urval av P-id:n, djup/sidor och motiverad balans.
- Utgångsläge: ny, partiell, rörig, kandidat, äldre avslut eller hinder.
- Resultatkrav: identitetsnivå eller livsbild; berörda PK-01–12 och avgränsningar.
- Frågor och källvägar: profilernas Q-/KP-id:n och materiell betydelse.
- Återanvändning: befintliga original, citationer, sökomfång och källkontexter.
- Avgränsning: tid, plats, serie/volym eller bestämd katalog-/källfråga.
- Utanför: uttryckliga gränser; nya generationer/andra frågor får egna tasks.
- Beroenden: fakta/nycklar/åtkomst, hur de kan lösas nu, Wotan-föregångare.

## Acceptance Criteria

- [ ] Varje berörd person har individuella forskningsobjekt och beläggslänkad
  disposition för tillämpliga PK-krav; mallfyllnad är inte saklig granskning.
- [ ] Fråga, alternativ och motiv för källval är prövade inom exakt omfång.
- [ ] Hela relevanta målposter och relationssammanhang är utvunna eller
  olästa delar exakt angivna; original, tolkning och provenans bevarade.
- [ ] Nya söknycklar har prövats mot berörda beroenden och anhörigprofiler;
  aktuella slutsatser, tidslinje och berättelse är konsoliderade.
- [ ] Positiva/negativa utfall och hinder har rätt räckvidd; ingen osäker
  identitet propagerar och OWNER_CONFIRMED bevaras.
- [ ] En versionerad operation/journalpost per batch; aktuella person-,
  forsknings- och täckningsbedömningar samt Wotan är samstämmiga. Beslutat följdarbete har egna avgränsade tasks före DONE.
- [ ] Relevanta sak- och strukturkontroller passerar; kunskapsresultat,
  begränsningar och vad task-DONE inte avslutar är uttryckliga.

## Approach

Förklara den kortaste motiverade kedjan av källor och vilka nycklar den kan
ge. Välj efter närhet/balans, identitetsrisk, vidare sökvärde och materiella
livsluckor. Källberoenden och alternativa åtkomstvägar får ändra ordningen.

## Återupptagning

- Uppdaterat:
- Utfört med belägg/batch:
- Delvis utfört och osäkerheter:
- Aktuella frågor, nya söknycklar och omprövade beroenden:
- Nästa ej utförda steg, exakt källa/sida/URL när tillämpligt:
- Verifiering, utfall och kvarvarande kontroll:
- Hinder, prövade alternativ och återaktiveringsvillkor:

## Verification

Sakgranska slutsatser mot lästa original/uppgiftslämnare, alternativa
identiteter och källberoenden. Kör kommandona i wotan/README.md och
uppgiftsspecifika kontroller; gröna tester är ingen bevisstandard.

## Outcome

Kunskapsresultat per fråga, effekt på identitet/livsbild/källstrategi,
kvarstående gränser, berörda andra profiler och Wotan-efterföljare.
