# T-NNNN: Konsolidera en fast personkohort

**Status**: READY | **Size**: M
**Phase**: -

## Context och mandat

PCD-2026-09-07-026; läs wotan/README.md, genealogy/person-standard.md och
personkontraktet samt genealogy2/docs/working.md. Detta är granskning av
befintlig kunskap. Aktstandardens sakkrav gäller; dess gamla Markdownformat
och skrivplatser ersätts av genealogy2:s aktuella vyer och operationer.

## Scope och underlag

- Fasta P-id:n, hushåll, djup/sidor och skälet till urvalet:
- Befintliga akter, profiler, assertioner, citationer och rättelser:
- Överlappande Wotan-arbete och vilka verifierade resultat som tillgodoräknas:
- Utanför: nya generationer, obegränsad arkivsökning och identitetssammanslagningar.
- Beroenden och konkret stoppgräns:

## Acceptance Criteria

- [ ] Alla namngivna akter har lästs tillsammans med relevant befintligt
  underlag, även senare rättelser som inte nått sammanfattningen.
- [ ] Varje akt och profil följer den pilotprövade standarden, med aktuella
  namn, datum, relationer, tidslinje, berättelse och kunskapsbedömningar.
- [ ] PK-01–12 och tio teman är individuellt motiverade; båda färdignivåerna
  behåller rätt innebörd. Kandidater och OWNER_CONFIRMED respekteras.
- [ ] Alla berörda anhöriga och beroenden har avstämts; avgränsat följdarbete
  utanför kohorten har namngiven Wotan-ägare före DONE.
- [ ] Varje person har ett granskningsutfall enligt tabellen nedan;
  rättelsekedjan och historiska observationer går att följa.
- [ ] Saklig eftergranskning samt relevanta obligatoriska kontroller passerar.

## Approach

1. Registrera faktiska avvikelser och vilka underlag som granskats per person.
2. Konsolidera och uppdatera profilen. Utvinn redan öppnade målposter enligt
   kontraktet; redovisa exakt oläst del om den inte går att kontrollera här.
3. Läs den färdiga akten igen mot beläggen och profilens slutsatser. Kontrollera
   särskilt påståenden om okänt/oläst, rättade datum, namn och anhöriga.
4. Spara batchen en gång i genealogy2:s operation/journal, med uppgiftens
   kriterium. Läs den härledda inventeringen och verifiera. Registrera
   konkret följdarbete i Wotan.

## Granskningsutfall

| Person | Datum/version | Granskat underlag | Format | Konsolidering | Identitetsbedömning | Livsbildsbedömning | Rättelser/luckor och följduppgift |
|---|---|---|---|---|---|---|---|

Fyll individuellt. Tabellen visar utfall och hänvisar till profilen; den
ersätter inte dess argument och är inte en andra kö.

## Verification

Kommandon och lästa utfall; saklig eftergranskning med beläggshänvisningar.

## Outcome

Vad blev mer tillförlitligt, vad är fortfarande okänt och vilken Wotan-uppgift
äger beslutat kvarstående arbete?

## Återupptagning

- Uppdaterat:
- Utfört med belägg/batch:
- Delvis utfört och osäkerheter:
- Aktuella frågor och berörda beroenden:
- Nästa ej utförda steg:
- Verifiering:
- Hinder:
