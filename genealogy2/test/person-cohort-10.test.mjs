...
test('tionde kohorten bevarar källrättelser och avgränsningar med full radtäckning, omkörning och återställning',async()=>{
 const dir=fs.mkdtempSync(path.join(os.tmpdir(),'genealogy2-persons10-test-'));
 ...
 try {
  ...
  // Det fastställda utgångsläget: journalposter1–35, inklusive de sju
  // källpreciseringarna och deras uttryckliga följdgranskningar.
  const entries=fs.readdirSync(path.join(base,'journal')).filter(f=>/^\d{9}-[a-f0-9]{64}\.json$/.test(f)&&Number(f.slice(0,9))<=35).sort();
  ...
  console.info('persons-10: bas, pilot och journalposter 1–35 återspelade');
  const {checkPersons10Risks}=await import('../verification/persons-10-risk-checks.mjs');
 const reviews=['a','b','c','d'].flatMap(k=>read(`migration/persons-10-${k}.json`)).sort((a,b)=>a.person.localeCompare(b.person));
 assert.equal(reviews.length,50);
 assert.deepEqual(reviews.map(r=>r.person),Array.from({length:50},(_,i)=>`P-${String(451+i).padStart(4,'0')}`));

  assert.equal(db.prepare('SELECT count(*) n FROM pending_review').get().n,0);
  const before=exportData(db);
  const operation=buildPersons(db,reviews,{group:'persons-10'});
  assert.equal(canonical(operation),canonical(read('operations/T-0661-persons-10-v1.json')));
  applyOperation(db,operation);
  assert.equal(applyOperation(db,operation).unchanged,true);
  assert.equal(checkPersons10Risks(db).ok,true);
  ...
  console.info('persons-10: införsel, omkörning, semantiska riskfall, historik och full enhetstäckning godkända');
  ...
  const views=['P-0453','P-0466'].map(id=>personView(db,id));
  ...
  assert.equal(checkPersons10Risks(restored).ok,true);
  assert.deepEqual(personView(restored,'P-0453'),views[0]);
  assert.deepEqual(personView(restored,'P-0466'),views[1]);
  console.info('persons-10: identisk återställning och bevarade faders-/civilståndskonflikter');
