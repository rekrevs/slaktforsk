...
import {checkPersons10PartBRisks} from '../../verification/persons-10-part-b-risk.mjs';
const source=openDB('genealogy2/data/research.sqlite',{readOnly:true});
const changes=new Map();
for(const part of ['a','b','c','d'])for(const p of JSON.parse(fs.readFileSync(`genealogy2/migration/persons-10-${part}.json`)))for(const x of p.changes)changes.set(x.id,x);
...
try {
  const own=JSON.parse(fs.readFileSync('genealogy2/migration/persons-10-b.json'));
  ...
  console.log(checkPersons10PartBRisks(fake));
