...
export function save(p) {const own=new Map(units(p.person,'person').filter(u=>u.kind==='assertion'&&!u.historical).map(u=>[u.legacy_id,u.id]));for(const m of [...p.changes,...p.reuse])m.origins=m.origins.map(o=>typeof o==='string'&&own.has(o)?own.get(o):o);const path='genealogy2/migration/persons-08-a.json',rows=fs.existsSync(path)?JSON.parse(fs.readFileSync(path)):[];if(rows.some(r=>r.person===p.person))throw Error('Redan sparad');rows.push(p);rows.sort((a,b)=>a.person.localeCompare(b.person));fs.writeFileSync(path,JSON.stringify(rows,null,2)+'\n');}
