# P-0141: Lovisa Svalberg, dotter i Malstugan, dopvittne 1881

## Arbetsläge

- Format: `person-dossier/v1`, konsoliderad 2026-09-10 i T-0184.
  **Minimiläge enligt PCD-2026-09-09-030:** löst anknuten person, kort text,
  ingen livsbildsforskning startas.
- Profil: [P-0141](../research-profiles/P-0141.md) — daterad PK-bedömning,
  tio teman, söknycklar och källvägar.
- Identitetsnivå **`UNDERKÄND`**, `Identitetsläge: OLÖST`;
  `Trädverkan: EJ BÄRANDE`. Full nivå `UNDERKÄND`.
- **Hennes led är postens enda med en relationsterm utan namngiven
  motpart.** `dotter` — dotter till vem? Källan säger det inte.
- **Ett andra Svalbergsled finns i samma rote**, i redan sparat bildmaterial:
  drängen `Johan Wilhelm Svalberg`, **född 1864-06-03 i Forssa**, på
  dopbarnets egen hushållssida. Se A-7820. **Ingen släktrelation registreras.**
- Aktens `## Relationer`-tabell hade **en tom rad inuti tabellen**; den är
  borttagen utan att någon rad gått förlorad.

## Identitet

Hemmavarande dotter i **Malstugan** i Forssa och dopvittne när
[Axel Edvard Eriksson](P-0015-axel-edvard-eriksson.md) döptes den 7 april 1881.

**Tre led står i klartext:** efternamnet `Svalberg`, förnamnet `Lovisa` och
familjeställningen `dotter`, plus boställsnamnet `Malstugan`.

**Men `dotter` hänger i luften.** Postens två andra parter binder sina
relationsled till en namngiven person — `H:u` till torparen Johan Peter
Pehrsson, och drängen till sitt boställe. **Hennes `dotter` namnger ingen
förälder**, och det är hennes leds egentliga svaghet: familjeställningen är
läst, men **den saknar motpart.**

**Ingen ålder finns.** Dopvittneskolumnen har ingen åldersruta. Att hon förs
som `dotter` och ändå var betrodd som dopvittne visar att hon var **myndig
eller nära myndig**, ingenting mer.

**Ett efternamn utan förälder och utan ålder identifierar ingen**, och
identitetsläget står därför `OLÖST`. **Vad som saknas är en andra källa, inte
en läsning** — Forssas husförhörslängd för **`Malstugan`** skulle ge både
föräldrarna och födelseåret.

**Namnet `Svalberg` är däremot ovanligt nog att bära.** Forssas
husförhörslängd `A I/16`, sida 83 — dopbarnets egen hushållssida — bär på
rad 11 drängen **`Johan Wilhelm Svalberg`, född 1864-06-03 i Forssa**,
inflyttad från Årdala 1881 och utflyttad till Wadsbro 1882 med attest 27.
**Två Svalberg i samma lilla rote är en söknyckel**, men **ingen
släktrelation registreras**: efternamnet är gemensamt, ingenting annat är
prövat, och hans egen inflyttning från Årdala anges först under 1881.

**Ingen släktrelation till dopbarnet anges**, och ingen registreras.

## Namnformer

| Form | Användning/tid | Status | Belägg | Kommentar |
|---|---|---|---|---|
| `Svalberg Lovisa` | 1881, dopvittneskolumnen | TRANSCRIBED | [C-0104](../citations/C-0104-axel-edvard-fodelse-1881.md) | **Källans form**, med efternamnet först enligt kolumnens konvention. |
| `Lovisa Svalberg` | aktens visningsnamn | TRANSCRIBED | [C-0104](../citations/C-0104-axel-edvard-fodelse-1881.md) | Normaliserad ordföljd av samma led. |
| `Louisa`, `Lovisa Svahlberg`, `Swalberg` | ej belagda | LEAD | — | **Måste ingå i varje sökning.** `v`/`w` och `a`/`ah` växlade fritt i efternamn på `-berg`. |
| ett patronymikon i stället för `Svalberg` | ej belagt | LEAD | — | **En hemmavarande dotter kunde föras med faderns familjenamn eller med patronymikon.** Om fadern hette Svalberg kan hon i en längd stå som `Larsdotter` eller motsvarande; **båda formerna måste tålas.** |

## Påståenden

| ID | Påstående | Status | Tillförlitlighet | Belägg | Kommentar |
|---|---|---|---|---|---|
| A-0572 | Namnet var Lovisa Svalberg och hon betecknas dotter i Malstugan. | TRANSCRIBED | hög | [C-0104](../citations/C-0104-axel-edvard-fodelse-1881.md) | Dopvittneskolumnen. |
| A-0573 | Hon var dopvittne 1881-04-07. | TRANSCRIBED | hög | [C-0104](../citations/C-0104-axel-edvard-fodelse-1881.md) | Dopvittneskolumnen. |
| A-7820 | **Hennes `dotter` namnger ingen förälder** — postens enda relationsterm utan motpart — och **ingen ålder finns.** Ett andra Svalbergsled står i samma rote: Forssa `A I/16` sida 83 bär på rad 11 drängen **`Johan Wilhelm Svalberg`, född 1864-06-03 i Forssa**, inflyttad från Årdala 1881 och utflyttad till Wadsbro 1882 med attest 27. **Efternamnet är en söknyckel; ingen släktrelation registreras.** | TRANSCRIBED | hög | [C-0104](../citations/C-0104-axel-edvard-fodelse-1881.md), [C-0149](../citations/C-0149-erik-anna-forssa-AI16-sida-83.md) | Vittnesledet är preciserat **utan ny bildläsning**; Svalbergsraden är ny läsning av den **redan sparade** bilden `C0006406_00086` 2026-09-10 i T-0184, ingen ny arkivsökning. **Endast efternamnet förenar dem** — inget föräldrapar, ingen ort och ingen ålder är gemensam, och hans inflyttning anges först 1881. **Ingen sammanslagning och inget nytt person-id.** T-0132:s drängförteckning i C-0149 anger hans födelseår `1862`; **den bevarade fullbilden ger `64 ³/₆`**, se citationens tillägg 2026-09-10. **Rättat i ett led samma dag:** **förnamnet läses `Johan`, inte `Ivan`** — se A-7837; observationen i övrigt står oförändrad. |
| A-7837 | **Drängens förnamn på Forssa `A I/16` sida 83 rad 11 läses `Johan`, inte `Ivan`.** Raden bär `Dr. Johan Wilhelm Svalberg`, och bokstavsformen är **identisk med `Johan` i rad 10:s `Carl Johan Pettersson`** på samma sida. | TRANSCRIBED | hög | [C-0149](../citations/C-0149-erik-anna-forssa-AI16-sida-83.md) | Omläsning av den **redan sparade** bilden `C0006406_00086` 2026-09-10 i T-0184, senare samma dag än A-7820; ingen ny arkivsökning. **A-7820 står kvar oförändrad** med sin ursprungliga läsning. **Rättelsen ändrar ingen slutsats om henne:** det är efternamnet `Svalberg` som är söknyckeln, och **ingen släktrelation registreras** — men förnamnet måste vara rätt i varje framtida sökning. |

## Relationer

| Person | Relation | Tid/plats | Status | Belägg |
|---|---|---|---|---|
| [P-0015 Axel Edvard Eriksson](P-0015-axel-edvard-eriksson.md) | **dopvittne vid hans dop**, ingen belagd släktrelation | Forssa 1881-04-07 | TRANSCRIBED | [C-0104](../citations/C-0104-axel-edvard-fodelse-1881.md) |
| [P-0139 Anna Ersdotter](P-0139-anna-ersdotter-dopvittne-1881.md), [P-0140 Carl Pettersson](P-0140-carl-pettersson-dopvittne-1881.md) | **postens övriga vittnesparter, från andra boställen**; ingen belagd relation till henne | Forssa 1881-04-07 | TRANSCRIBED | [C-0104](../citations/C-0104-axel-edvard-fodelse-1881.md) |
| en icke namngiven förälder i Malstugan | **`dotter` utan namngiven motpart** — familjeställningen är läst, föräldern finns inte i källan | Malstugan i Forssa, 1881 | LEAD | [C-0104](../citations/C-0104-axel-edvard-fodelse-1881.md) |
| `Johan Wilhelm Svalberg` i samma rote | **enbart gemensamt efternamn** — ingen prövad släktrelation, **inget person-id skapat** | Svartsjötorp i Forssa, 1881–1882 | LEAD | [C-0149](../citations/C-0149-erik-anna-forssa-AI16-sida-83.md) |

## Tidslinje

| Datum/intervall | Händelse | Plats | Roll/yrke | Hushåll | Belägg |
|---|---|---|---|---|---|
| 1881-04-07 | **dopvittne**, som hemmavarande dotter | Forssa | — | i Malstugan, husbonden ej namngiven | [C-0104](../citations/C-0104-axel-edvard-fodelse-1881.md) |
| före och efter 1881 | **helt obelagt** | — | — | — | — |

**Aktens hela kronologi är en dag, och den dagen är dopbarnets.**

## Boställen och flyttar

| Tid | Från/till eller boställe | Fastighet/adress | Belägg | Kommentar |
|---|---|---|---|---|
| 1881-04-07 | **`Malstugan`** | i Forssa, i Malstanäs rote | [C-0104](../citations/C-0104-axel-edvard-fodelse-1881.md) | **Boställsnamnet står på hennes eget led**, till skillnad från P-0139:s. Roten bar flera stugor under Malstanäs — folkräkningen 1880 för `Nystugan`, `Röda stugan` och `Hagstugan` **till Malstanäs** — och **`Malstugan` är ingen av dem.** Formen bevaras oförändrad. |
| — | inga flyttningar | — | [C-0104](../citations/C-0104-axel-edvard-fodelse-1881.md) | En dopnotis har ingen flyttningskolumn. |

## Yrke, utbildning och ekonomi

| Tid | Uppgift | Plats/organisation | Belägg | Kommentar |
|---|---|---|---|---|
| 1881 | **ingen egen ståndsbeteckning** | Malstugan | [C-0104](../citations/C-0104-axel-edvard-fodelse-1881.md) | `dotter` är en familjeställning och **inte ett yrke**. Till skillnad från P-0140, som bär `dräng`. |
| — | ingen ekonomisk uppgift | — | [C-0104](../citations/C-0104-axel-edvard-fodelse-1881.md) | En dopnotis har ingen sådan kolumn. Ingen yta öppnas i minimiläge. |

## Militärt, civilt och rättsligt

| Tid | Uppgift | Förband/myndighet/mål | Belägg | Kommentar |
|---|---|---|---|---|
| 1881-04-07 | **betrodd som dopvittne** | Forssa församling | [C-0104](../citations/C-0104-axel-edvard-fodelse-1881.md) | Rollen förutsatte att hon var myndig eller nära myndig. |
| — | **inget civilstånd angivet** | Malstugan | [C-0104](../citations/C-0104-axel-edvard-fodelse-1881.md) | Att hon förs som `dotter` i ett föräldrahushåll talar för ogift, **men posten anger inget civilstånd** och slutsatsen dras inte som ett belägg. |
| — | ingen militär eller rättslig uppgift | — | [C-0104](../citations/C-0104-axel-edvard-fodelse-1881.md) | Saknas i källformen. |

## Hälsa och död

| Tid | Uppgift | Källa/belägg | Källkritisk kommentar |
|---|---|---|---|
| 1881-04-07 | levde och var närvarande vid ett dop | [C-0104](../citations/C-0104-axel-edvard-fodelse-1881.md) | Enda livstecknet. |
| — | **ingen ålder och inget födelseår** | [C-0104](../citations/C-0104-axel-edvard-fodelse-1881.md) | Dopvittneskolumnen har **ingen åldersruta.** **En källgräns, inte en läsosäkerhet.** |
| — | ingen dödsuppgift | — | En dopnotis har ingen dödskolumn. **Men boställsnamnet står på hennes eget led**, vilket gör en husförhörsrad nåbar. |

## Media och personliga källor

| Objekt | Datum | Upphov/provenans | Citation | Lokal fil |
|---|---|---|---|---|
| Forssa `C/6`, bild 30, fullbild | läst 2026-08-20, fullpostprövad 2026-09-08 i T-0132 | Riksarkivet, `F0002785_00030`, 6 704 × 5 424 px | [C-0104](../citations/C-0104-axel-edvard-fodelse-1881.md) | [C/6 bild 30](../media/C-0104-riksarkivet-SE-ULA-10279-C6-bild-30-axel-edvard-1881.jpg) |
| Forssa `A I/16`, bild 86, sida 83, fullbild | läst 2026-08-21, drängraderna utvunna 2026-09-10 i T-0184 | Riksarkivet, `C0006406_00086`, 7 232 × 6 288 px | [C-0149](../citations/C-0149-erik-anna-forssa-AI16-sida-83.md) | [A I/16 bild 86](../media/C-0149-riksarkivet-forssa-AI16-bild-86-sida-83.jpg) |

Inget personligt material är känt.

## Biografisk sammanfattning

Den 7 april 1881 stod Lovisa Svalberg, hemmavarande dotter i **Malstugan**,
som tredje och sista vittnespart när Axel Edvard Eriksson döptes i Forssa.
Före henne kom torparparet från Näfverkärr och drängen från Svartgjöl.

**Hennes led bär en relation utan motpart.** `dotter` — men källan namnger
ingen förälder, och det är den enda av postens tre relationstermer som inte
binds till en namngiven person. Ingen ålder finns, för dopvittneskolumnen har
ingen åldersruta.

**Efternamnet är däremot ovanligt nog att bära.** På dopbarnets egen
hushållssida i Forssas husförhörslängd står drängen `Johan Wilhelm Svalberg`,
född 3 juni 1864 i Forssa. **Två Svalberg i samma lilla rote är en
söknyckel** — men bara efternamnet förenar dem, och ingen släktrelation
registreras.

**En dotter i ett namngivet hus är ändå lättare att nå än en förkortning.**
Forssas husförhörslängd för `Malstugan` omkring 1881 skulle ge både hennes
föräldrar och hennes födelseår.

## Forskningsfrågor och konflikter

1. **Vilka var hennes föräldrar, och när föddes hon?** Forssas
   husförhörslängd omkring 1881, uppslaget för **`Malstugan`**, skulle ge
   båda. Se profilens Q-01, KP-01.
2. **Hur förhåller hon sig till `Johan Wilhelm Svalberg` i samma rote?**
   Enbart efternamnet förenar dem. Samma uppslag skulle avgöra det. Se Q-02.
3. **`Malstugan` är ingen av folkräkningens tre stugor under Malstanäs.**
   Formen bevaras oförändrad och normaliseras inte mot `Hagstugan`,
   `Nystugan` eller `Röda stugan`.
4. **Relationen till dopbarnet står inte i källan**, och ingen släkthypotes
   registreras.
5. **Ingen konflikt kvarstår i akten.**

## Historik och rättelser

Akten fördes till `person-dossier/v1` 2026-09-10 i T-0184, i minimiläge enligt
PCD-2026-09-09-030. Före det bestod den av rubriken, ett tvåraders
`## Identitet`, en `## Påståenden`-tabell med två rader, en
`## Relationer`-tabell med en rad och en enradig forskningsfråga; ingen
tidslinje, inga namnformer och ingen profil fanns.

**Båda ursprungliga assertionerna står kvar med oförändrad lydelse.**

#### Tom rad inuti `## Relationer`-tabellen borttagen, 2026-09-10

Tabellen hade en **tom rad mellan avgränsarraden och sin enda datarad**, ett
formatfel som bröt tabellen i två. Raden är borttagen; **ingen datarad har
gått förlorad.** Samma fel fanns i
[P-0140](P-0140-carl-pettersson-dopvittne-1881.md).

#### Drängens förnamn rättat: `Johan`, inte `Ivan`, 2026-09-10

A-7820 skriver drängen på Forssa `A I/16` sida 83 rad 11 som
**`Ivan Wilhelm Svalberg`**. **Vid omläsning samma dag läses förnamnet
`Johan`**, med en bokstavsform identisk med `Johan` i rad 10:s
`Carl Johan Pettersson` på samma sida. Se A-7837 och citationens tillägg.

**A-7820 står kvar oförändrad**, och **rättelsen ändrar ingen slutsats om
henne**: söknyckeln är efternamnet `Svalberg`, och ingen släktrelation är
eller blir registrerad. **Aktens löptext använder den rättade formen.**

#### Aktens rubriknamn utökat, 2026-09-10

Rubriken löd `P-0141: Lovisa Svalberg`. Den lyder nu
`P-0141: Lovisa Svalberg, dotter i Malstugan, dopvittne 1881`. **Filnamnet är
oförändrat.** Ingen akt har delats och ingen identitet har slagits samman.

#### Tidigare arbetsläge, historiskt

Akten hade inget arbetsläge, ingen granskningsstatus och ingen profil före
2026-09-10. Ingen äldre GRANSKAD- eller KLAR-bedömning har konverterats.
